...
class vuln(object):
	...
	def __init__(self):
		try:
			self.vuln_name = "zerodium Vulnerability"
			self.stage = "zerodium_STAGE1"
	...
	def incoming(self, message, bytes, ip, vuLogger, random_reply, ownIP):
		try:
			### logging object
			self.log_obj = amun_logging.amun_logging("vuln_zerodium", vuLogger)
			...
			if self.stage == "zerodium_STAGE1" and message.startswith('GET / HTTP/1.1'):
